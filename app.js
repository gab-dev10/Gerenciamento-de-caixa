...
let tiposLavagem = DB.get('lj_tipos', [
  { id: 't1', nome: 'Simples', preco: 30, ativo: true },
  { id: 't2', nome: 'Completa', preco: 60, ativo: true },
  { id: 't3', nome: 'Polimento', preco: 120, ativo: true },
  { id: 't4', nome: 'Higienização', preco: 200, ativo: true }
]);
let categorias = DB.get('lj_cats', [
  { id: 'c1', nome: 'Produtos de Limpeza', ativo: true },
  { id: 'c2', nome: 'Manutenção', ativo: true },
  { id: 'c3', nome: 'Aluguel', ativo: true },
  { id: 'c4', nome: 'Combustível', ativo: true },
  { id: 'c5', nome: 'Outros', ativo: true }
]);
...
function getPeriodDates() {
  ...
  if (currentPeriod === 'semana') {
    ...
    start = new Date(now); start.setDate(now.getDate() - day + (day === 0 ? -6 : 1));
...
function populateFormSelects() {
  ...
  const sd = document.getElementById('d-categoria');
  const fst = document.getElementById('f-s-tipo');
  const fdc = document.getElementById('f-d-cat');
 
  [st, fst].forEach(el => {
    if (!el) return;
    el.innerHTML = '<option value="">Selecionar tipo</option>';
  ...
  if (fdc) {
    fdc.innerHTML = '<option value="">Todas as categorias</option>';
    categorias.filter(c => c.ativo).forEach(c => {
      fdc.innerHTML += `<option value="${c.id}">${c.nome}</option>`;
    });
  }
}
...
function salvarServico() {
  ...
  const obj = {
    id: editingId || uid(), data, cliente, carro,
    tipo_id: tipo, preco, pagamento, status: currentStatus,
    data_pagamento: currentStatus === 'pago' ? today() : null,
    obs, criado_em: editingId ? undefined : today()
  };
...
function cancelEdit() {
  ...
  limparFormServico(); limparFormDespesa();
}
...
function refreshDashboard() {
  ...
  const receita = servPeriod.filter(s => s.status === 'pago').reduce((a, s) => a + s.preco, 0);
  const desp = despPeriod.reduce((a, d) => a + d.valor, 0);
  const lucro = receita - desp;
  const lavagens = servPeriod.length;
  const areceber = servPeriod.filter(s => s.status === 'nao-pago').reduce((a, s) => a + s.preco, 0);
  const pagos = servPeriod.filter(s => s.status === 'pago');
  ...
  // PENDENCIAS
  const pendentes = servicos.filter(s => s.status === 'nao-pago').sort((a, b) => a.data.localeCompare(b.data));
  ...
  // CHARTS
  buildCharts(servPeriod, despPeriod, start, end);
}
...
function buildCharts(servs, desps, start, end) {
  ...
  // Gera labels de dias entre start e end (max 31)
  const days = [];
  ...
  // Receita x Despesas
  mkChart('chartRecDesp', 'bar', { labels, datasets: [
  ...
  ]}, { ...opt, plugins: { legend: { display: true, labels: { color: '#8896b0', boxWidth: 12 } } } });
 
  // Evolução lavagens
  const lavDay = days.map(d => servs.filter(s => s.data === d).length);
  mkChart('chartLavagens', 'line', { labels: days.map(d => { const [,m,day] = d.split('-'); return `${day}/${m}`; }), datasets: [{
    data: lavDay, borderColor: '#60a5fa', backgroundColor: 'rgba(96,165,250,0.1)',
    fill: true, tension: 0.4, pointRadius: 3
  }]}, opt);
 
  // Por dia da semana
  const dias = ['Dom','Seg','Ter','Qua','Qui','Sex','Sáb'];
  const recSemana = dias.map((_, i) => servs.filter(s => s.status === 'pago' && new Date(s.data + 'T00:00:00').getDay() === i).reduce((a,s) => a+s.preco, 0));
  mkChart('chartDiaSemana', 'bar', { labels: dias, datasets: [{
    data: recSemana, backgroundColor: 'rgba(167,139,250,0.7)', borderRadius: 6
  }]}, opt);
  ...
  }, { ...opt, plugins: { legend: { display: true, position: 'bottom', labels: { color: '#8896b0', boxWidth: 10 } } } });
  ...
  mkChart('chartDespesasCat', 'doughnut', {
    ...
    datasets: [{ data: Object.values(catVal), backgroundColor: pizzaColors.slice().reverse(), borderWidth: 0 }]
  }, { ...opt, plugins: { legend: { display: true, position: 'bottom', labels: { color: '#8896b0', boxWidth: 10 } } } });
}
...
function marcarPago(id) {
  openModal('Confirmar Pagamento', `Marcar este serviço como pago? A data de pagamento será registrada como hoje (${fmtDate(today())}).`, () => {
    const s = servicos.find(x => x.id === id);
    if (s) { s.status = 'pago'; s.data_pagamento = today(); }
...
function renderHistorico() {
  ...
  if (histTab === 'servicos') {
    ...
    if (busca) data = data.filter(s => s.cliente.toLowerCase().includes(busca) || s.carro.toLowerCase().includes(busca));
    total = data.filter(s => s.status === 'pago').reduce((a,s) => a+s.preco,0);
 
    items = data.map(s => {
      const t = tiposLavagem.find(x => x.id === s.tipo_id);
      return `<div class="hist-item">
        <div class="dot ${s.status === 'pago' ? 'green' : 'yellow'}"></div>
        <div class="hist-info">
          <div class="hist-main">${s.cliente} — ${s.carro}</div>
          <div class="hist-sub">${fmtDate(s.data)} · ${t ? t.nome : '—'} · ${s.pagamento || '—'}</div>
        </div>
        <span class="badge ${s.status === 'pago' ? 'pago' : 'nao-pago'}">${s.status === 'pago' ? 'Pago' : 'Não Pago'}</span>
  ...
  list.innerHTML = items.length ? items.join('') : '<div class="empty-state">📭 Nenhum registro encontrado</div>';
}
 
function editarServico(id) {
  const s = servicos.find(x => x.id === id);
  if (!s) return;
  editingId = id; editingType = 'servico';
  goPage('lancamentos');
  setTipo('servico');
  setTimeout(() => {
...
function editarDespesa(id) {
  ...
  if (!d) return;
  editingId = id; editingType = 'despesa';
  goPage('lancamentos');
  setTipo('despesa');
  setTimeout(() => {
...
function exportCSV() {
  ...
  let csv = 'SERVIÇOS\n';
  csv += 'Data,Cliente,Carro,Tipo,Preço,Pagamento,Status\n';
  sp.forEach(s => {
    const t = tiposLavagem.find(x => x.id === s.tipo_id);
    csv += `${s.data},${s.cliente},${s.carro},${t ? t.nome : ''},${s.preco},${s.pagamento || ''},${s.status}\n`;
  });
...
function exportPrint() {
  ...
  <style>body{font-family:sans-serif;padding:20px;color:#111}table{width:100%;border-collapse:collapse;margin-bottom:20px}th,td{padding:8px;border:1px solid #ddd;font-size:0.85rem}th{background:#f0f0f0}h2{margin-top:24px}@media print{.no-print{display:none}}</style>
  </head><body>
  <h1>Relatório — LavaJato</h1>
  <p>Gerado em ${new Date().toLocaleDateString('pt-BR')}</p>
  <h2>Resumo Financeiro</h2>
  <table><tr><th>Receita Total</th><th>Despesas</th><th>Lucro Líquido</th></tr>
  <tr><td>${fmt(receita)}</td><td>${fmt(desp)}</td><td>${fmt(receita-desp)}</td></tr></table>
  <h2>Serviços</h2><table><tr><th>Data</th><th>Cliente</th><th>Carro</th><th>Tipo</th><th>Valor</th><th>Status</th></tr>
  ${sp.map(s => { const t = tiposLavagem.find(x=>x.id===s.tipo_id); return `<tr><td>${fmtDate(s.data)}</td><td>${s.cliente}</td><td>${s.carro}</td><td>${t?t.nome:''}</td><td>${fmt(s.preco)}</td><td>${s.status}</td></tr>`; }).join('')}
  </table><h2>Despesas</h2><table><tr><th>Data</th><th>Categoria</th><th>Descrição</th><th>Valor</th></tr>
  ${dp.map(d => { const c = categorias.find(x=>x.id===d.categoria_id); return `<tr><td>${fmtDate(d.data)}</td><td>${c?c.nome:''}</td><td>${d.descricao}</td><td>${fmt(d.valor)}</td></tr>`; }).join('')}
  </table><button class="no-print" onclick="window.print()">🖨️ Imprimir</button>
  </body></html>`);
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('modal-confirm').addEventListener('click', e => { if (e.target.id === 'modal-confirm') closeModal(); });
